...
const seedDatabase = async () => {
  try {
    ...
    await Lesson.create({ title: 'Leçon 1: npm init et express()', content: 'On commence par initialiser npm et importer express.', order: 1, module: m1._id });
    await Lesson.create({ title: 'Leçon 1: Mon premier JSX', content: 'Le JSX permet d\'écrire du HTML dans le code JS.', order: 1, module: m2._id });
    await Lesson.create({ title: 'Leçon 1: insertOne et find', content: 'Utilisation des commandes basiques mongo shell.', order: 1, module: m3._id });
    await Lesson.create({ title: 'Leçon 1: Le fichier package.json', content: 'Comprendre les dépendances et scripts Node.', order: 1, module: m4._id });
    await Lesson.create({ title: 'Leçon 1: int, String, boolean', content: 'Déclarer des variables simples en Java.', order: 1, module: m5._id });
    await Lesson.create({ title: 'Leçon 1: Afficher du texte avec echo', content: 'La fonction echo permet de générer du HTML.', order: 1, module: m6._id });
    await Lesson.create({ title: 'Leçon 1: Qu\'est-ce qu\'un Endpoint ?', content: 'Définition d\'une URL d\'accès API.', order: 1, module: m7._id });
    await Lesson.create({ title: 'Leçon 1: Ecrire SELECT * FROM table', content: 'Récupérer toutes les données d\'une table SQL.', order: 1, module: m8._id });
    await Lesson.create({ title: 'Leçon 1: Découverte de MaterialApp', content: 'La structure de base d\'une app Flutter.', order: 1, module: m9._id });
    await Lesson.create({ title: 'Leçon 1: Syntaxe avec les accolades {{}}', content: 'Afficher une variable dans le template.', order: 1, module: m10._id });

    
    const qz1 = await Quiz.create({ course: c1._id, title: 'Quiz Express.js', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t1._id });
    const qz2 = await Quiz.create({ course: c2._id, title: 'Quiz React 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t2._id });
    const qz3 = await Quiz.create({ course: c3._id, title: 'Quiz MongoDB 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t3._id });
    const qz4 = await Quiz.create({ course: c4._id, title: 'Quiz Node.js 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t4._id });
    const qz5 = await Quiz.create({ course: c5._id, title: 'Quiz Java 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t5._id });
    const qz6 = await Quiz.create({ course: c6._id, title: 'Quiz PHP 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t6._id });
    const qz7 = await Quiz.create({ course: c7._id, title: 'Quiz API REST 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t7._id });
    const qz8 = await Quiz.create({ course: c8._id, title: 'Quiz SQL 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t8._id });
    const qz9 = await Quiz.create({ course: c9._id, title: 'Quiz Flutter 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t9._id });
    const qz10 = await Quiz.create({ course: c10._id, title: 'Quiz Angular 101', description: 'Test de niveau basique', duration: 10, passingScore: 50, isPublished: true, createdBy: t10._id });

    
    const qn1_1 = await Question.create({ quiz: qz1._id, statement: 'Quelle méthode Express écoute sur un port donné ?', type: 'MCQ', points: 2, order: 1 });
    const qn1_2 = await Question.create({ quiz: qz1._id, statement: 'Express est un framework pour Node.js.', type: 'TrueFalse', points: 2, order: 2 });
    const qn1_3 = await Question.create({ quiz: qz1._id, statement: 'Quel package est utilisé pour gérer les routes dans Express ?', type: 'ShortAnswer', points: 2, order: 3 });

    
    const qn2_1 = await Question.create({ quiz: qz2._id, statement: 'Quel hook permet d\'avoir un état local dans un composant ?', type: 'MCQ', points: 2, order: 1 });
    const qn2_2 = await Question.create({ quiz: qz2._id, statement: 'Le JSX est obligatoire pour utiliser React.', type: 'TrueFalse', points: 2, order: 2 });
    const qn2_3 = await Question.create({ quiz: qz2._id, statement: 'Quelle méthode rend un élément React dans le DOM ?', type: 'ShortAnswer', points: 2, order: 3 });

   
    const qn3_1 = await Question.create({ quiz: qz3._id, statement: 'MongoDB stocke les données sous quel format principal ?', type: 'MCQ', points: 2, order: 1 });
    const qn3_2 = await Question.create({ quiz: qz3._id, statement: 'MongoDB est une base de données relationnelle.', type: 'TrueFalse', points: 2, order: 2 });
    const qn3_3 = await Question.create({ quiz: qz3._id, statement: 'Quelle commande permet de lire tous les documents ?', type: 'ShortAnswer', points: 2, order: 3 });

    
    const qn4_1 = await Question.create({ quiz: qz4._id, statement: 'Quel est le gestionnaire de paquets par défaut de Node.js ?', type: 'MCQ', points: 2, order: 1 });
    const qn4_2 = await Question.create({ quiz: qz4._id, statement: 'Node.js exécute le code du côté serveur.', type: 'TrueFalse', points: 2, order: 2 });
    const qn4_3 = await Question.create({ quiz: qz4._id, statement: 'Quel objet global permet d\'accéder aux variables d\'environnement ?', type: 'ShortAnswer', points: 2, order: 3 });

    
    const qn5_1 = await Question.create({ quiz: qz5._id, statement: 'Quel mot-clé est utilisé pour déclarer une classe en Java ?', type: 'MCQ', points: 2, order: 1 });
    const qn5_2 = await Question.create({ quiz: qz5._id, statement: 'Java est un langage compilé et interprété.', type: 'TrueFalse', points: 2, order: 2 });
    const qn5_3 = await Question.create({ quiz: qz5._id, statement: 'Quelle est la méthode principale de point d\'entrée en Java ?', type: 'ShortAnswer', points: 2, order: 3 });

    
    const qn6_1 = await Question.create({ quiz: qz6._id, statement: 'Par quel symbole commencent les variables en PHP ?', type: 'MCQ', points: 2, order: 1 });
    const qn6_2 = await Question.create({ quiz: qz6._id, statement: 'PHP s\'exécute dans le navigateur du client.', type: 'TrueFalse', points: 2, order: 2 });
    const qn6_3 = await Question.create({ quiz: qz6._id, statement: 'Quelle fonction permet d\'afficher du texte en PHP ?', type: 'ShortAnswer', points: 2, order: 3 });

    
    const qn7_1 = await Question.create({ quiz: qz7._id, statement: 'Quelle méthode HTTP sert à envoyer de nouvelles données ?', type: 'MCQ', points: 2, order: 1 });
    const qn7_2 = await Question.create({ quiz: qz7._id, statement: 'GET doit modifier les données sur le serveur.', type: 'TrueFalse', points: 2, order: 2 });
    const qn7_3 = await Question.create({ quiz: qz7._id, statement: 'Quel code de statut HTTP indique un succès (OK) ?', type: 'ShortAnswer', points: 2, order: 3 });

    
    const qn8_1 = await Question.create({ quiz: qz8._id, statement: 'Quel mot-clé SQL permet de récupérer des enregistrements ?', type: 'MCQ', points: 2, order: 1 });
    const qn8_2 = await Question.create({ quiz: qz8._id, statement: 'La clause WHERE permet de filtrer les résultats.', type: 'TrueFalse', points: 2, order: 2 });
    const qn8_3 = await Question.create({ quiz: qz8._id, statement: 'Quel mot-clé permet de supprimer des lignes dans SQL ?', type: 'ShortAnswer', points: 2, order: 3 });

    
    const qn9_1 = await Question.create({ quiz: qz9._id, statement: 'Quel langage de programmation est utilisé dans Flutter ?', type: 'MCQ', points: 2, order: 1 });
    const qn9_2 = await Question.create({ quiz: qz9._id, statement: 'Tout est Widget dans Flutter.', type: 'TrueFalse', points: 2, order: 2 });
    const qn9_3 = await Question.create({ quiz: qz9._id, statement: 'Quel widget est utilisé pour afficher du texte ?', type: 'ShortAnswer', points: 2, order: 3 });

    
    const qn10_1 = await Question.create({ quiz: qz10._id, statement: 'Quel langage est principalement utilisé avec Angular ?', type: 'MCQ', points: 2, order: 1 });
    const qn10_2 = await Question.create({ quiz: qz10._id, statement: 'Angular utilise un DOM virtuel comme React.', type: 'TrueFalse', points: 2, order: 2 });
    const qn10_3 = await Question.create({ quiz: qz10._id, statement: 'Quel symbole entoure les variables dans les templates Angular ?', type: 'ShortAnswer', points: 2, order: 3 });
    ...
    const att1 = await QuizAttempt.create({ student: s1._id, quiz: qz1._id, score: 6, totalQuestions: 3, duration: 300 });
    const att2 = await QuizAttempt.create({ student: s2._id, quiz: qz2._id, score: 6, totalQuestions: 3, duration: 300 });
    const att3 = await QuizAttempt.create({ student: s3._id, quiz: qz3._id, score: 6, totalQuestions: 3, duration: 300 });
    const att4 = await QuizAttempt.create({ student: s4._id, quiz: qz4._id, score: 6, totalQuestions: 3, duration: 300 });
    const att5 = await QuizAttempt.create({ student: s5._id, quiz: qz5._id, score: 6, totalQuestions: 3, duration: 300 });
    const att6 = await QuizAttempt.create({ student: s6._id, quiz: qz6._id, score: 6, totalQuestions: 3, duration: 300 });
    const att7 = await QuizAttempt.create({ student: s7._id, quiz: qz7._id, score: 6, totalQuestions: 3, duration: 300 });
    const att8 = await QuizAttempt.create({ student: s8._id, quiz: qz8._id, score: 6, totalQuestions: 3, duration: 300 });
    const att9 = await QuizAttempt.create({ student: s9._id, quiz: qz9._id, score: 6, totalQuestions: 3, duration: 300 });
    const att10 = await QuizAttempt.create({ student: s10._id, quiz: qz10._id, score: 6, totalQuestions: 3, duration: 300 });

    
    await Answer.create({ attempt: att1._id, question: qn1_1._id, selectedChoice: ch1_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att1._id, question: qn1_2._id, selectedChoice: ch1_3._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att1._id, question: qn1_3._id, textAnswer: 'express-router', isCorrect: true, pointsEarned: 2 });

    
    await Answer.create({ attempt: att2._id, question: qn2_1._id, selectedChoice: ch2_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att2._id, question: qn2_2._id, selectedChoice: ch2_4._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att2._id, question: qn2_3._id, textAnswer: 'createRoot', isCorrect: true, pointsEarned: 2 });

    
    await Answer.create({ attempt: att3._id, question: qn3_1._id, selectedChoice: ch3_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att3._id, question: qn3_2._id, selectedChoice: ch3_4._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att3._id, question: qn3_3._id, textAnswer: 'find', isCorrect: true, pointsEarned: 2 });

    
    await Answer.create({ attempt: att4._id, question: qn4_1._id, selectedChoice: ch4_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att4._id, question: qn4_2._id, selectedChoice: ch4_3._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att4._id, question: qn4_3._id, textAnswer: 'process.env', isCorrect: true, pointsEarned: 2 });

    
    await Answer.create({ attempt: att5._id, question: qn5_1._id, selectedChoice: ch5_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att5._id, question: qn5_2._id, selectedChoice: ch5_3._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att5._id, question: qn5_3._id, textAnswer: 'main', isCorrect: true, pointsEarned: 2 });

    
    await Answer.create({ attempt: att6._id, question: qn6_1._id, selectedChoice: ch6_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att6._id, question: qn6_2._id, selectedChoice: ch6_4._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att6._id, question: qn6_3._id, textAnswer: 'echo', isCorrect: true, pointsEarned: 2 });

    
    await Answer.create({ attempt: att7._id, question: qn7_1._id, selectedChoice: ch7_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att7._id, question: qn7_2._id, selectedChoice: ch7_4._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att7._id, question: qn7_3._id, textAnswer: '200', isCorrect: true, pointsEarned: 2 });

    
    await Answer.create({ attempt: att8._id, question: qn8_1._id, selectedChoice: ch8_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att8._id, question: qn8_2._id, selectedChoice: ch8_3._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att8._id, question: qn8_3._id, textAnswer: 'DELETE', isCorrect: true, pointsEarned: 2 });

    await Answer.create({ attempt: att9._id, question: qn9_1._id, selectedChoice: ch9_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att9._id, question: qn9_2._id, selectedChoice: ch9_3._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att9._id, question: qn9_3._id, textAnswer: 'Text', isCorrect: true, pointsEarned: 2 });

    await Answer.create({ attempt: att10._id, question: qn10_1._id, selectedChoice: ch10_1._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att10._id, question: qn10_2._id, selectedChoice: ch10_4._id, isCorrect: true, pointsEarned: 2 });
    await Answer.create({ attempt: att10._id, question: qn10_3._id, textAnswer: '{{ }}', isCorrect: true, pointsEarned: 2 });
